...
class ShowBook : public wxFrame{
   ...
   // SetSize(wxSize(800, 600));
    ...
    // Set column labels
    grid->SetColLabelValue(0, "BOOK LABEL");
    grid->SetColLabelValue(1, "BOOK NAME");
    grid->SetColLabelValue(2, "AUTHOR");
    grid->SetColLabelValue(3, "PUBLISHER");
    grid->SetColLabelValue(4, "PRICE");
    grid->SetColLabelValue(5, "RELEASE DATE");
    grid->SetColLabelValue(6, "PAGE NUMBER");
    grid->SetColLabelValue(7, "RECEIVE DAY");
    grid->SetColLabelValue(8, "CONDITION");
    ...
    *******************************************/
    void AddToFile(wxCommandEvent& event){
        ...
        if (fs::exists(BookExist)) {
            ...
            number.clear();
            receipt.clear();
            wxMessageBox("BOOK THAT ALREADY EXIST", "WARNING", wxOK | wxICON_ERROR);

            return;
        }else if(label.IsEmpty() || name.IsEmpty() || author.IsEmpty() || publisher.IsEmpty() || price.IsEmpty() || date.IsEmpty() || number.IsEmpty() || receipt.IsEmpty()){
            wxMessageBox("PLEASE ENTER INFO", "WARNING", wxOK | wxICON_ERROR);
            label.clear();
            name.clear();
            author.clear();
            publisher.clear();
        ...
        file.open(FileName);
        file << left << setw(30) << labelString << setw(30) << nameString << setw(30) << authorString << setw(30) << publisherString << setw(30) << priceString << setw(30) << dateString<< setw(30) << numberString << setw(30) << receiptString  << 0 << endl;
        file.close();
        wxMessageBox("SUCCESSFUL", "NOTICE", wxOK | wxICON_ERROR);
        labelString.clear();
        nameString.clear();
        authorString.clear();
        publisherString.clear();
        priceString.clear();
        dateString.clear();
        numberString.clear();
        receiptString.clear();
...
class DeleteBook : public wxFrame{
    ...
    *******************************************/
    ...
    if (!file) {
        wxMessageBox("CANNOT OPEN FILE", "WARNING", wxOK | wxICON_ERROR);
        return;
    }
    ...
    if (!fileContent.empty() && fileContent.back() == '0') {
        ...
        wxMessageBox("SUCCESSFUL", "NOTICE", wxOK | wxICON_INFORMATION);
    ...
    *******************************************/
    void ShowBookLabel(wxCommandEvent& event){
        ...
        wxMessageDialog dialog(this, fileList, "LIST OF BOOKS", wxOK | wxICON_INFORMATION);
        dialog.ShowModal();
    }
    private:
        wxTextCtrl* BookLabel;
    ...
    *******************************************/
    void CheckLogin(wxCommandEvent& event) {
        ...
        if (username == line1 && password == line2) {
            ...
            passwordInput->Clear();
            if (Attempt > 0) {
                wxMessageBox("WRONG ID OR PASSWORD", "WARNING", wxOK | wxICON_ERROR);
            }
            Attempt--;
        }
        if (login) {
    ...
    public:
        ...
        *******************************************/
        void SAVEINFO(wxCommandEvent& event){
            ...
            a.Address = address;
            listUser.insert(a);
            wxMessageBox("SAVED", "NOTICE", wxOK | wxICON_ERROR);
            address.clear();
            phone.clear();
            //listUser.print();
        }
    ...
    *******************************************/
    void Muon(wxCommandEvent& event) {
        ...
        if (!inputFile.is_open()) {
            wxMessageBox("CANNOT CREATE FILE", "WARNING", wxOK | wxICON_ERROR);
        ...
        if (!duLieu.empty()) {
            ...
            wxMessageBox("BOOK HAS BEEN BORROWED", "NOTICE", wxOK | wxICON_ERROR);
        ...
        if (canMuonSach) {
            ...
            wxMessageBox("CANNOT BORROW", "NOTICE", wxOK | wxICON_ERROR);
        ...
        if (!outputFile.is_open()) {
            wxMessageBox("CANNOT CREATE TICKET", "WARNING", wxOK | wxICON_ERROR);
        ...
        outputFile.close();
        //printSuccessful();
        wxMessageBox("TICKET SUCCESSFULLY CREATED", "NOTICE", wxOK | wxICON_ERROR);
...
class RestoreDisplay : public wxFrame{
    ...
    *******************************************/
    void RestoreFunction(wxCommandEvent& event){
        ...
        if (!file.is_open()) {
            wxMessageBox("CANNOT FIND TICKET", "WARNING", wxOK | wxICON_ERROR);
            return;
        }
        string line;
        ...
        if (!inputFile.is_open()) {
            wxMessageBox("CANNOT FIND BOOK", "WARNING", wxOK | wxICON_ERROR);
            return;
        }
        string duLieu((istreambuf_iterator<char>(inputFile)), istreambuf_iterator<char>());
        bool canMuonSach = true;
        ...
        if (canMuonSach) {
            ...
            wxMessageBox("BOOK HAS BEEN BORROWED", "WARNING", wxOK | wxICON_ERROR);
            return;
        }
        ofstream putFile(fileBook, ios::trunc);
        if (!putFile.is_open()) {
            wxMessageBox("SYSTEM ERROR", "WARNING", wxOK | wxICON_ERROR);
            return;
        }
        putFile << duLieu;
        putFile.close();
        if (daThayThe) {
            wxMessageBox("SUCCESSFULLY RETURN BOOK", "NOTICE", wxOK | wxICON_ERROR);
        } else {
            wxMessageBox("BOOK HAS BEEN RETURN BEFORE", "WARNING", wxOK | wxICON_ERROR);
        }
    }
    private:
...
public:
   Member(const wxString& title) : wxFrame(NULL, wxID_ANY, title) {
    ...
    grid->SetColLabelValue(3, "PHONE NUMBER");
    grid->SetColLabelValue(4, "DAY");
